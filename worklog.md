...
Task ID: 24
Agent: Main Agent
Task: Phase 3.5-3.6 布局和实验列表组件迁移

**日期**: 2025-03-05

**背景**: 继续将组件从 AppContext 迁移到 React Query

### Work Log:

#### 1. Dashboard 组件迁移
- 从 AppContext 迁移到 useExperiments, useProjects, useAuth
- 添加加载状态处理
- 保持所有原有功能（统计、提醒、最近记录等）

#### 2. Header 组件迁移
- 从 AppContext 迁移到 useAuth
- 简化依赖，仅使用 currentUser

#### 3. Sidebar 组件迁移
- 从 AppContext 迁移到 useAuth, useLogout, useExperiments, useProjects
- 任务数量计算使用 React Query 数据
- logout 使用 useLogout mutation

#### 4. ExperimentList 组件迁移
- 从手动 fetch 迁移到 useExperiments hook
- 支持 viewMode 参数
- 移除 65 行代码，简化逻辑
...
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| src/components/dashboard/Dashboard.tsx | 重构 | React Query 迁移 |
| src/components/layout/Header.tsx | 重构 | React Query 迁移 |
| src/components/layout/Sidebar.tsx | 重构 | React Query 迁移 |
| src/components/experiments/ExperimentList.tsx | 重构 | React Query 迁移 |

### 迁移进度统计:
| 组件 | 状态 | 使用的技术 |
|------|------|-----------|
| TemplateList | ✅ 已迁移 | React Query |
| Dashboard | ✅ 已迁移 | React Query |
| Header | ✅ 已迁移 | React Query |
| Sidebar | ✅ 已迁移 | React Query |
| ExperimentList | ✅ 已迁移 | React Query |
| FileManager | ✅ 已拆分 | 自定义 Hook |
| MyTasks | ⏳ 待迁移 | 独立状态 |
| ProjectList | ⏳ 待迁移 | 独立状态 |
| ReviewList | ⏳ 待迁移 | AppContext |
| ExperimentDetail | ⏳ 待迁移 | AppContext |
| ExperimentEditor | ⏳ 待迁移 | AppContext |

### Git提交记录:
```
691945f refactor: Phase 3.6 ExperimentList 组件迁移到 React Query
653895a refactor: Phase 3.5 布局组件迁移到 React Query
```

---

## Task ID: 21 - Phase 3.5 组件迁移到 React Query

**日期**: 2025-03-28

**背景**: 继续进行架构优化，将更多组件从 AppContext 迁移到 React Query

### 架构设计

最终采用 **AppContext + React Query 共存架构**:

| 状态类型 | 管理方式 | 原因 |
|---------|---------|------|
| 服务端数据 (模板、实验、项目等) | React Query | 自动缓存、刷新、重试 |
| 认证状态 (currentUser) | AppContext | 需要全局访问，与 API 无关 |
| UI 状态 (侧边栏、模态框) | AppContext | 纯客户端状态 |
| 用户偏好 (主题、设置) | AppContext | 本地持久化 |

### Work Log:

#### 1. useProjects hooks 更新
- 新增 `viewMode` 参数支持
- 新增 `useDeleteProject` hook
- 新增 `useCreateProject` hook

#### 2. 新增 useUnlockRequests hook
- `useUnlockRequests` - 获取解锁申请列表
- `useProcessUnlockRequest` - 处理解锁申请（批准/拒绝）

#### 3. ProjectList 组件迁移
- 从 `useApp` 迁移到 `useProjects`/`useAuth` hooks
- 移除手动状态管理
- 使用 React Query 的 isLoading 和 refetch

#### 4. MyTasks 组件迁移
- 从 `useApp` 迁移到 `useExperiments`/`useProjects`/`useAuth` hooks
- 使用 `useUnlockRequests` 获取解锁申请数据

#### 5. types.tsx 更新
- 更新为使用 `hooks/api` 中的类型
- 移除对 AppContext 类型的依赖

### 迁移状态总结:

| 组件 | 状态 | 技术 |
|------|------|------|
| Dashboard | ✅ 已迁移 | useAuth, useExperiments, useProjects |
| ExperimentList | ✅ 已迁移 | useAuth, useExperiments |
| TemplateList | ✅ 已迁移 | useTemplates |
| FileManager | ✅ 已拆分 | 自定义 Hook |
| ProjectList | ✅ 本次迁移 | useProjects, useAuth |
| MyTasks | ✅ 本次迁移 | useExperiments, useProjects, useAuth |
| ReviewList | ⚠️ 已废弃 | 功能合并到 MyTasks |

### AppContext 现保留内容:
- 认证状态 (currentUser, login, logout, register)
- Token 管理 (authFetch, getStoredToken, setStoredToken, clearStoredToken)

### 文件变更:
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| src/hooks/api/useProjects.ts | 修改 | 新增 viewMode、useDeleteProject、useCreateProject |
| src/hooks/api/useUnlockRequests.ts | 新建 | 解锁申请管理 hooks |
| src/hooks/api/index.ts | 修改 | 导出新 hooks |
| src/components/projects/ProjectList.tsx | 重构 | 迁移到 React Query |
| src/components/tasks/MyTasks.tsx | 重构 | 迁移到 React Query |
| src/components/tasks/types.tsx | 修改 | 使用 hooks/api 类型 |

### Git提交记录:
```
4a4bad7 refactor: Phase 3.5 组件迁移到 React Query
```

### Stage Summary:
- ✅ useProjects hooks 扩展完成
- ✅ useUnlockRequests hook 创建完成
- ✅ ProjectList 迁移完成
- ✅ MyTasks 迁移完成
- ✅ Lint 检查通过
- ✅ 已推送到远程仓库
...
## Task ID: 22 - 修复 React Query queryKey 格式问题

**日期**: 2025-03-28

**背景**: 用户报告控制台错误 - React Query v4/v5 要求 queryKey 必须是数组格式
...
**文件**: `src/hooks/api/useUnlockRequests.ts`
...
1. `['experiments']` 不能正确 invalidate 使用 `['experiments', 'list']` 作为 key 的缓存
2. 使用统一的 `experimentKeys.lists()` 确保缓存失效的一致性

### Git 提交
```
1660d1d fix: 修复 React Query queryKey 格式问题
```

### Stage Summary:
- ✅ queryKey 格式问题已修复
- ✅ 使用统一的 key factory 模式
- ✅ Lint 检查通过
- ✅ 已推送到远程仓库


---
## Task ID: 21 - React Query queryKey 格式错误修复

**日期**: 2025-03-05

**背景**: 用户报告 React Query v4 queryKey 格式错误："As of v4, queryKey needs to be an Array"

### 问题分析

#### 问题根源
在 `src/hooks/api/useAuth.ts` 中，`authKeys` 对象存在属性名冲突：

```typescript
export const authKeys = {
  user: [currentUser] as const,       // 静态属性 - 数组
  users: [users] as const,
  user: (id: string) => [users, id] as const,  // 函数 - 覆盖了上面的静态属性！
}
```

后面的 `user: (id: string) => ...` 函数覆盖了前面的 `user: [currentUser]` 数组属性。

当代码调用 `authKeys.user` 期望获取数组时，实际得到的是一个函数，导致 React Query 报错。
...
#### 2. 更新所有引用
- `useAuth()` - 使用 `authKeys.currentUser`
- `useUsers()` - 使用 `authKeys.allUsers`
- `useLogin()` - 使用 `authKeys.currentUser`
- `useRegister()` - 使用 `authKeys.currentUser`

### 文件变更:
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| src/hooks/api/useAuth.ts | 修改 | 重命名 authKeys 属性避免冲突 |
...
**背景**: 用户指出当前模块文档中缺少"普通视角"和"管理员全局视角"的说明，需要补充这部分内容
...
#### 1. 创建视角说明补充方案文档
- 新建 `docs/VIEW_PERSPECTIVE_SUPPLEMENT.md`
- 定义视角模式概念：默认视角 vs 全局视角
- 分析各模块视角差异
- 制定文档补充清单

#### 2. 更新用户手册 USER_GUIDE.md
- 新增"视角模式"完整章节
- 包含：什么是视角模式、如何切换、各模块差异、使用建议、注意事项
...
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| docs/VIEW_PERSPECTIVE_SUPPLEMENT.md | 新建 | 视角说明补充方案 |
| docs/USER_GUIDE.md | 修改 | 新增视角模式章节 |
| docs/modules/experiments.md | 修改 | 新增视角差异章节 |
| docs/modules/projects.md | 修改 | 新增视角差异章节 |
| docs/modules/my-tasks.md | 修改 | 新增视角差异章节 |
| docs/modules/review.md | 修改 | 新增视角差异章节 |
| docs/modules/permissions.md | 修改 | 完善全局视角技术实现 |
| docs/ARCHITECTURE.md | 修改 | 新增视角系统架构章节 |
...
---
## Task ID: 23 - 仪表盘与数据中心设计方案讨论

**日期**: 2025-03-06

**背景**: 讨论仪表盘和实验模板两个相对独立模块的功能策划方向
...
- 方案一：两个独立界面
- 方案二：视角切换
- **最终决策**：混合方案（仪表盘 + 数据中心两个独立页面）

#### 2. 混合方案优势
- 职责分离清晰：仪表盘=个人工作台，数据中心=管理驾驶舱
- 符合现有模式：用户管理、文件管理已是管理员独立入口
- 管理员双重身份自然体现
- 实现简单，无需复杂视角切换逻辑
...
#### 1. 分析当前实现
- Dashboard.tsx：统计卡片、暂存提醒、待审核提醒、最近实验、项目概览
- TemplateList.tsx：CRUD、富文本、公开/私有、标签分类

#### 2. 设计方案讨论
- 确定仪表盘定位：个人工作台（所有用户）
- 确定数据中心定位：管理驾驶舱（管理员专属）
- 规划各模块功能清单

#### 3. 创建设计草案文档
- 文件：`docs/design/DASHBOARD_AND_DATACENTER_DESIGN.md`
- 内容：方案概述、模块设计、技术规划、待确认事项、版本规划

### 文件变更:
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| docs/design/DASHBOARD_AND_DATACENTER_DESIGN.md | 新建 | 设计方案草案 |

### Stage Summary:
- ✅ 确定混合方案方向
- ✅ 设计草案文档已创建
- ✅ 待确认事项已整理（12项）
- ✅ 版本规划建议已完成（v3.4.0-v3.6.0）
- ⏳ 待后续确认细节后开发

### Git提交:
```
541ef39 docs: 新增仪表盘与数据中心设计方案草案
```

---
## Task ID: 24 - 实验规范中心设计方案讨论

**日期**: 2025-03-06

**背景**: 讨论实验模板模块的转型方向，确定轻量化改造方案

### 讨论要点

#### 1. 问题分析
- 实验记录已改为上传附件为主，在线编辑模板价值弱化
- 原模板模块与新工作流不匹配
...
- 方案二：模板作为实验创建配置
- 方案三：暂停/弱化模板
- **最终决策**：转型为"实验规范中心"，包含SOP库和模板共享

#### 3. 权限设计确认
- SOP库：管理员上传管理，所有人查看下载
- 模板共享：大家可上传，上传者和管理员可删除

#### 4. 设计原则
- 轻量级：无审核流程、无版本管理、无实验关联
- 线上：文档存放、分类检索、便捷获取
- 线下：审核修改、内容更新
...
#### 3. 创建设计草案文档
- 文件：`docs/design/STANDARD_CENTER_DESIGN.md`
- 内容：页面设计、对话框设计、数据模型、API设计、开发任务清单

### 文件变更:
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| docs/design/STANDARD_CENTER_DESIGN.md | 新建 | 设计方案草案 |

### Stage Summary:
- ✅ 确定转型方案：实验模板 → 实验规范中心
- ✅ 确定权限设计：SOP管理员管理，模板大家共享
- ✅ 确定轻量化原则：移除富文本编辑
- ✅ 设计草案文档已创建
- ✅ 待确认事项已确定（5项）
- ⏳ 待后续开发
