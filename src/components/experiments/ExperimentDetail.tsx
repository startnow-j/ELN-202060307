...
export function ExperimentDetail({ experiment: initialExperiment, onEdit, onBack }: ExperimentDetailProps) {
  const { deleteExperiment, currentUser, triggerExtraction, updateExtractedInfo, submitForReview, viewMode } = useApp()
  ...
  }, [initialExperiment.id, currentUser])
  ...
  // 是否是全局视角
  const isGlobalView = viewMode === 'global' && isAdmin
  
  // 基础权限检查
  // - 普通视角：只有作者可以编辑自己的实验
  // - 全局视角：管理员可以编辑所有人的实验
  const baseCanEdit = (experiment.reviewStatus === 'DRAFT' || experiment.reviewStatus === 'NEEDS_REVISION') &&
                  (isGlobalView || currentUser?.id === experiment.authorId)
