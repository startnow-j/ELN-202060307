...
function needsProjectReminder(experiment: Experiment): boolean {
  return (experiment as any).storageLocation === 'draft' || 
         (!(experiment as any).storageLocation && experiment.projects?.length === 0)
}
...
export function ExperimentList({ onCreateExperiment, onViewExperiment }: ExperimentListProps) {
  ...
  // 使用 React Query 获取实验数据
  const { data: experiments = [], isLoading, refetch } = useExperiments({ viewMode })

  // 分离项目相关和暂存实验
  const projectExperiments = experiments.filter(exp => 
    (exp as any).storageLocation !== 'draft' && exp.projects?.length > 0
  )
  
  const draftExperiments = experiments.filter(exp => 
    (exp as any).storageLocation === 'draft' || (!(exp as any).storageLocation && exp.projects?.length === 0)
  )
  ...
  const canCreateExperiment = (() => {
    ...
    for (const project of userProjects) {
      const myMembership = (project as any).members?.find((m: any) => m.id === currentUser.id)
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {viewMode === 'global' && isAdmin ? '管理所有用户的实验记录（全局视角）' : '管理您的所有实验记录'}
          </p>
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                    <span>作者: {experiment.author?.name || '未知'}</span>
