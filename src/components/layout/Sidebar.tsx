...
export function Sidebar({ activeTab, onTabChange }: SidebarProps) {
  ...
  // AI配置菜单（仅超级管理员可见）
  const showAIConfigMenu = currentUser?.role === 'SUPER_ADMIN'
      ...
      <ScrollArea className="flex-1 py-4">
        <nav className="space-y-1 px-2">
          ...
          <Button
            ...
            className={cn(
              "w-full justify-start gap-3 text-sidebar-foreground relative",
          ...
          {showAIConfigMenu && (
            <Button
              ...
              className={cn(
                "w-full justify-start gap-3 text-sidebar-foreground",
                activeTab === 'ai-config' && "bg-sidebar-accent text-sidebar-accent-foreground font-medium",
                collapsed && "justify-center px-2"
              )}
              onClick={() => onTabChange('ai-config')}
            >
              <Bot className="w-5 h-5 flex-shrink-0" />
              {!collapsed && <span>AI配置</span>}
            </Button>
          )}
        </nav>
      </ScrollArea>
