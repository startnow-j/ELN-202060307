...
export async function GET(
  ...
  try {
    ...
    for (const ep of experiment.experimentProjects) {
      ...
      // 项目所有者可以审核（排除作者和当前用户自己）
      if (project.owner && project.ownerId !== experiment.authorId && project.ownerId !== userId) {
      ...
      // 项目负责人角色可以审核（排除作者和当前用户自己）
      for (const member of project.projectMembers) {
        if (member.role === 'PROJECT_LEAD' && member.userId !== experiment.authorId && member.userId !== userId) {
    ...
    const admins = await db.user.findMany({
      where: {
        ...
        isActive: true,
        id: { notIn: [experiment.authorId, userId] }
      },
