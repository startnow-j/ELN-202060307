...
import { secureAICall } from '@/lib/ai-security'
...
// 系统提示词
const SYSTEM_PROMPT = `你是一个专业的实验室记录分析助手。请从实验记录中提取关键信息。
...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      // 获取激活的AI配置来确定provider
      const activeConfig = await db.aIConfig.findFirst({
        where: { isActive: true }
      })

      if (!activeConfig) {
        throw new Error('AI服务未配置，请联系超级管理员在"AI配置"中添加配置')
      }

      console.log(`[AI Extract] Using AI config: ${activeConfig.provider} / ${activeConfig.modelName}`)

      // 调用AI进行提取（使用安全封装，关联AI配置模块）
      ...
      if (result.sanitizedItems && result.sanitizedItems.length > 0) {
        console.log(`[AI Extract] Sanitized items: ${result.sanitizedItems.join(', ')}`)
      }
