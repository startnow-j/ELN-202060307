...
export async function POST(
  ...
  try {
    ...
    // 检查权限：作者可以申请解锁，管理员在全局视角下可以代为申请解锁
    const isAuthor = experiment.authorId === userId
    const canRequestAsAdmin = isAdminUser && isGlobalView

    if (!isAuthor && !canRequestAsAdmin) {
      return NextResponse.json({ error: '只有作者才能申请解锁，或在全局视角下由管理员代为申请' }, { status: 403 })
    }
